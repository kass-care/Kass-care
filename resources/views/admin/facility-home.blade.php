        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            @if(auth()->user()->facility_id)
                <form method="POST" action="/select-facility/{{ auth()->user()->facility_id }}">
                    @csrf
                    <button type="submit"
                            class="inline-flex items-center rounded-2xl border border-indigo-400 px-5 py-3 text-sm font-semibold text-indigo-100">
        ...
        <div class="lg:col-span-2 rounded-3xl bg-slate-900 border border-slate-800 p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="rounded-2xl bg-slate-950 border border-slate-800 p-5">
                    ...
                    <p class="text-2xl font-semibold mt-3">
                        {{ auth()->user()->facility?->email ?? auth()->user()->email ?? 'Not provided' }}
                    </p>
                </div>
            </div>
        </div>
