...
<div class="max-w-3xl mx-auto px-6 py-10">
    <div class="bg-white rounded-2xl shadow-sm border border-indigo-100 p-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Caregiver</h1>
                <p class="text-gray-600 mt-1">Update caregiver account details.</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.caregivers.update', $caregiver->id) }}" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Full Name</label>
                <input type="text" name="name" value="{{ old('name', $caregiver->name) }}"
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email', $caregiver->email) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500"
                       required>
            </div>
                ...
                <select name="facility_id"
                    ...
                    @foreach($facilities as $facility)
                        <option value="{{ $facility->id }}"
                            {{ old('facility_id', $caregiver->facility_id) == $facility->id ? 'selected' : '' }}>
                            {{ $facility->name }}
                        </option>
                    @endforeach
                </select>
