...
<div class="max-w-7xl mx-auto py-8 px-4">
    ...
    {{-- Keep the rest of your summary page content below this line --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl font-bold text-gray-900">Clinical Snapshot</h2>

            @php
                $riskClasses = match($intelligence['risk_level'] ?? 'LOW') {
            ...
            <span class="inline-flex items-center rounded-full px-4 py-2 text-sm font-bold {{ $riskClasses }}">
                Risk Level: {{ $intelligence['risk_level'] ?? 'LOW' }}
            </span>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div class="rounded-xl bg-gray-50 border border-gray-100 p-4">
                ...
                <div class="mt-2 text-lg font-bold text-gray-900">
                    {{ !empty($snapshot['lastVisit']) ? \Carbon\Carbon::parse($snapshot['lastVisit']->visit_date)->format('M d, Y') : 'N/A' }}
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            @if($latestCareLog && $latestCareLog->created_at)
        ...
        @if($latestCareLog)
            @php
                ...
                $bp = $vitals['bp'] ?? $vitals['blood_pressure'] ?? 'N/A';
                $pulse = $vitals['pulse'] ?? 'N/A';
                $temp = $vitals['temperature'] ?? $vitals['temp'] ?? 'N/A';
                $oxygen = $vitals['oxygen'] ?? $vitals['oxygen_saturation'] ?? 'N/A';
            @endphp
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            @forelse($client->medications as $med)
                <div class="py-3 border-b border-gray-100 last:border-b-0">
                    <div class="font-semibold text-gray-900">
